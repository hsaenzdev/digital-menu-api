...
export const orderPlugin = new Elysia({ prefix: '/api/orders' })
  ...
  .post('/', async ({ body }) => {
    try {
      ...
      // For bank transfers, set the transfer reference to the order number
      let finalOrder = order
      if (paymentMethod === 'bank_transfer') {
        finalOrder = await prisma.order.update({
          where: { id: order.id },
          data: { transferReference: `ORD-${order.orderNumber}` },
          include: {
            customer: true,
      ...
      return {
        success: true,
        data: parseOrderData(finalOrder),
        message: `Order #${finalOrder.orderNumber} created successfully`
      }
    } catch (error) {
